...
class table_home_follow 
{
	...
	public function fetch_all_by_followuid($followuid, $status = 0, $start = 0, $limit = 0) {
		$data = array();
		$wherearr = array();
		$force = !$start && !$limit && !$status ? false : true;
		if((!$force && ($data = $this->fetch_cache($uid)) === false) || $force) {
			$parameter = array($this->_table, $followuid);
			$wherearr[] = 'followuid=%d';
			$wheresql = !empty($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
			$data = DB::fetch_all("SELECT * FROM %t $wheresql ORDER BY dateline DESC ".DB::limit($start, $limit), $parameter, 'uid');
			
			if(!$force) {
				$this->store_cache($followuid, $data, $this->_cache_ttl);
			}
		}
	...
	public function is_friend($uid,$followuid){
		$res =  DB::fetch_first ( "SELECT count(*) as count FROM %t WHERE uid= %d AND followuid = %d",array('home_follow',$uid,$followuid));
		return $res['count'];
	}
}
