...
class memberProfile
{
	...
	public function get_profile_by_uid($uids, $file, $start = 0, $limit = 10) {
		...
		return DB::fetch_all ( "SELECT c.stars,b.groupid,a.uid," . $file . " FROM %t AS a left join %t AS b ON a.uid=b.uid left join %t AS c ON b.groupid=c.groupid WHERE 1=1 " . $where . 'ORDER BY c.stars DESC' .DB::limit ( $start, $limit ), $parameter, 'uid' );
	}
	...
	public static function update($data,$condition){
		return DB::update('common_member_profile', $data, $condition);
	}
}
