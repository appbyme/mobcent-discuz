...
class table_home_notification
{
	...
	public static function count_by_uid($uid,$new, $type)
	{
		$new = intval($new);
		$type = $type ? ' AND ('.$type.')': '';
		$new = ' AND '.DB::field('new', $new);
		return DB::fetch_first("SELECT count(*) as num  FROM %t WHERE uid=%d %i %i ORDER BY new DESC, dateline DESC %i", array(self::$tablename, $uid, $type, $new, DB::limit($start, $perpage)));
	}
	...
	public static function get_notification($uid,$type,$start,$perpage){
		$sql = 'SELECT * FROM %t WHERE uid = %d AND type=%s ORDER BY dateline desc';
		$arr = array('home_notification',$uid,$type, DB::limit ($start,$perpage));
		return DB::fetch_all($sql,$arr);
	}
	public static function notification_add($username ,$touid, $uid,$type, $note, $notevars = array(), $system = 0) {
		global $_G;
